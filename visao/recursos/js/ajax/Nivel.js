function salvarNivel() {
    if ($("#nomeNivel").val() !== null && $("#nomeNivel").val() !== "") {
        $.ajax({
    ...
    } else if ($("#nomeNivel").val() === null || $("#nomeNivel").val() === "") {
        swal("Atenção", "Por favor defina um nome para o nível!", "info");
    }
}
...
function procurarPerfilAcesso(codnivel) {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            dataTablePadrao('tabela_perfil_acesso');
        }, error: function (jqXHR, textStatus, errorThrown) {
            swal("Erro", "Erro causado por:" + errorThrown, "error");
        }
    });
